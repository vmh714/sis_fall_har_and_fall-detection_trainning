...
OUT_DIR = CURRENT_DIR / 'train_v14_kq_6ch'
...
def main():
    ...
    # --- CẮT DỮ LIỆU CÒN 6 TRỤC (IMU GỐC) ---
    ...
    checkpoint_path = OUT_DIR / 'best_model_v14_6ch.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v14 (6 kênh - IMU)...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v14_6ch.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v14 (6 Trục - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn - TCN v14 (6 Channels)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v14_6ch.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v14_6ch.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
