...
OUT_DIR = CURRENT_DIR / 'train_v15_kq'
...
def build_tcn_mcu(input_shape, n_classes=4):
    ...
    # Trở về GlobalAveragePooling1D
    x = GlobalAveragePooling1D()(x)
    
    # THÊM MỚI v15: Lớp Dense phi tuyến để tăng độ tinh tế khi phân loại Walk vs Static
    x = Dense(64, activation='relu', name='dense_hidden')(x)
    x = BatchNormalization(name='bn_hidden')(x)  # Thêm BN để ổn định quá trình huấn luyện
    x = Dropout(0.3)(x)
    
    outputs = Dense(n_classes, activation='softmax', name='output_layer')(x)
    model = Model(inputs=inputs, outputs=outputs)
    
    # sparse_categorical_crossentropy (Bỏ Label Smoothing)
...
def main():
    ...
    # v15: Giữ mức phạt x3.0 cho Fall để đảm bảo chuẩn y tế
    fall_idx = CLASS_NAMES.index('Fall')
    class_weights[fall_idx] *= 3.0
    
    # v15: GIẢM HỆ SỐ WALK XUỐNG 1.05 ĐỂ TRÁNH LỆCH BIAS NHƯ v14
    walk_idx = CLASS_NAMES.index('Walk')
    class_weights[walk_idx] *= 1.05
    
    input_shape = (X_train.shape[1], X_train.shape[2])
    model = build_tcn_mcu(input_shape, n_classes=len(CLASS_NAMES))
    
    checkpoint_path = OUT_DIR / 'best_model_v15.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v15...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v15.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v15 (Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) - TCN v15')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v15.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v15.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
