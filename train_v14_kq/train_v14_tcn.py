...
OUT_DIR = CURRENT_DIR / 'train_v14_kq'
...
def build_tcn_mcu(input_shape, n_classes=4):
    ...
    # Trở về GlobalAveragePooling1D của v12
    x = GlobalAveragePooling1D()(x)
    x = Dropout(0.3)(x)
    outputs = Dense(n_classes, activation='softmax', name='output_layer')(x)
    model = Model(inputs=inputs, outputs=outputs)
    
    # Trở về sparse_categorical_crossentropy (Bỏ Label Smoothing)
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v14.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v14...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v14.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v14 (Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) - TCN v14')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v14.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v14.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
        
    # tflite_path = OUT_DIR / 'model_v14_int8.tflite'
