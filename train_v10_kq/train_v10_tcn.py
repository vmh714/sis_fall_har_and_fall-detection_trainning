...
OUT_DIR = CURRENT_DIR / 'train_v10_kq'
...
def build_tcn_mcu(input_shape, n_classes=4):
    ...
    for stack in range(2):
        for d in [1, 2, 4, 8]:
            residual = x
            x = Conv1D(32, kernel_size=3, dilation_rate=d, padding='causal', activation='relu', use_bias=True)(x)
...
def main():
    ...
    # Ở v10, ta duy trì mức phạt x5 tương tự v8 để có gốc accuracy cao
    ...
    checkpoint_path = OUT_DIR / 'best_model_v10.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v10...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v10.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v10 (Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) - TCN v10')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v10.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v10.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
        
    tflite_path = OUT_DIR / 'model_v10_int8.tflite'
