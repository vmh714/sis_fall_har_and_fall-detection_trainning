...
def build_model(input_shape):
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=5e-4), # [CÁCH 3]: Giảm từ 1e-3 xuống 5e-4),
...
def main():
    ...
    checkpoint_path = CURRENT_DIR / 'best_model_v3.keras'
    callbacks = [
    ...
    plt.tight_layout()
    plt.savefig(CURRENT_DIR / 'training_history_v3.png')
    plt.close()
    print("[*] Đã vẽ và lưu đồ thị lịch sử huấn luyện thành 'training_history_v3.png'")
    ...
    # Báo cáo kết quả
    print("\n" + "="*50)
    print("BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET)")
    print("="*50)
    ...
    # Vẽ Confusion Matrix đẹp mắt bằng Matplotlib thuần túy (không cần seaborn)
    ...
    plt.tight_layout()
    plt.savefig(CURRENT_DIR / 'confusion_matrix_v3.png')
    plt.close()
    print("[*] Đã vẽ và lưu ma trận nhầm lẫn thành 'confusion_matrix_v3.png'")
    ...
    print("\n" + "="*50)
    print(f"KẾT QUẢ ĐÁNH GIÁ CHUYÊN BIỆT LỚP TÉ NGÃ (FALL):")
    print(f"  - Số ca ngã thực tế trong tập Test: {true_falls}")
    print(f"  - Số ca ngã mô hình phát hiện đúng: {detected_falls}")
    print(f"  - TỶ LỆ RECALL PHÁT HIỆN TÉ NGÃ:    {recall_fall:.2f}%")
    if recall_fall >= 95.0:
        print("  => ĐẠT TIÊU CHUẨN! (Recall >= 95%)")
    else:
        print("  => CẦN CẢI THIỆN! (Recall < 95%)")
    print("="*50 + "\n")
