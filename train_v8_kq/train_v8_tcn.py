...
from tensorflow.keras.layers import Input, Conv1D, GlobalAveragePooling1D, Dense, Dropout, BatchNormalization, Add
...
OUT_DIR = CURRENT_DIR / 'train_v8_kq'
...
def export_to_tflite_int8(keras_model, X_train, tflite_path):
    ...
    # Representative dataset để lượng tử hóa
    # Dùng 200 mẫu ngẫu nhiên từ tập train
    ...
    print(f"[*] Kích thước mô hình: {len(tflite_model) / 1024:.2f} KB")
...
def main():
    ...
    # Phạt nặng việc đoán sai nhãn Fall (index 3) để đẩy mạnh Recall
    fall_idx = CLASS_NAMES.index('Fall')
    class_weights[fall_idx] *= 5.0
    print(f"[*] Trọng số cân bằng các lớp (Class Weights) sau khi điều chỉnh phạt Fall: {class_weights}")
    
    input_shape = (X_train.shape[1], X_train.shape[2])
    model = build_tcn_mcu(input_shape, n_classes=len(CLASS_NAMES))
    
    checkpoint_path = OUT_DIR / 'best_model_v8.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v8...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v8.png')
    plt.close()
    print("[*] Đã vẽ và lưu đồ thị lịch sử huấn luyện thành 'training_history_v8.png'")
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v8\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) trên Test Set - TCN v8')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v8.png')
    plt.close()
    
    report_str += "\n[*] Đã vẽ và lưu ma trận nhầm lẫn thành 'confusion_matrix_v8.png'\n"
    ...
    with open(OUT_DIR / 'report_v8.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
    print(f"[*] Đã lưu toàn bộ báo cáo vào {OUT_DIR / 'report_v8.txt'}")
    
    # 8. Export to TFLite Full INT8
    tflite_path = OUT_DIR / 'model_v8_int8.tflite'
