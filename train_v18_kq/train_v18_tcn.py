...
OUT_DIR = CURRENT_DIR / 'train_v18_kq'
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v18.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v18 (5 lớp - float32 - C++ firmware)...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v18.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v18 (5 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v18 (5 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v18.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v18.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
