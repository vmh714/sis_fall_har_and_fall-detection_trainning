...
OUT_DIR = CURRENT_DIR / 'train_v19_kq'
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v19.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v19 (với kiến trúc v14, 5 lớp)...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v19.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v19 (5 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v19 (5 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v19.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v19.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
