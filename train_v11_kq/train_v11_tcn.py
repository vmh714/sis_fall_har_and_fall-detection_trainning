...
OUT_DIR = CURRENT_DIR / 'train_v11_kq'
...
def build_tcn_mcu(input_shape, n_classes=4):
    ...
    # Sử dụng một lớp Conv1D ban đầu để chuyển số kênh từ 9 lên 32
    # Việc này giúp cố định chiều dài chuỗi là 200 và số kênh là 32 trước khi vào vòng lặp TCN
    ...
    for stack in range(2):
        for d in [1, 2, 4, 8]:
            ...
            # Nhánh chính: Tính toán trích xuất đặc trưng theo dilation
            x = Conv1D(32, kernel_size=3, dilation_rate=d, padding='same', activation='relu', use_bias=True)(x)
            x = BatchNormalization()(x)
            x = Dropout(0.2)(x)
            
            # Nhánh tắt (Shortcut): Lúc này cả x và residual đều đã có cùng số channel (32)
            # Nếu có bất kỳ sự lệch nhỏ nào về chiều dài chuỗi do phép toán dilation gây ra,
            # lớp Conv1D kernel=1 dưới đây sẽ ép chiều dài chuỗi của residual khớp hoàn toàn với x
            if residual.shape[1] != x.shape[1] or residual.shape[-1] != x.shape[-1]:
...
def main():
    ...
    # Ở v11, ta duy trì mức phạt x5 tương tự v8 để có gốc accuracy cao
    ...
    checkpoint_path = OUT_DIR / 'best_model_v11.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v11...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v11.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v11 (Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) - TCN v11')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v11.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v11.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
