...
OUT_DIR = CURRENT_DIR / 'train_v24_kq'
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v24.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình MCU-Optimized TCN v24 (Strides + Pooling + No Dilation)...")
    history = model.fit(
        X_train, y_train_oh,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v24.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v24 (5 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v24 (5 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v24.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v24.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
