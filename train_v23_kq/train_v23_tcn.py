...
from tensorflow.keras.layers import Input, Conv1D, GlobalAveragePooling1D, GlobalMaxPooling1D, Concatenate, Dense, Dropout, BatchNormalization, Add, Multiply, Reshape, MaxPooling1D
...
OUT_DIR = CURRENT_DIR / 'train_v23_kq'
...
def build_tcn_mcu(input_shape, n_classes=5):
    ...
    for i in range(4):
        ...
        x = Conv1D(32, kernel_size=7, strides=s, padding='same', activation='relu', use_bias=True)(x)
        x = BatchNormalization()(x)
        x = Dropout(0.2)(x)
        
        x = Conv1D(32, kernel_size=7, strides=1, padding='same', activation='relu', use_bias=True)(x)
        x = BatchNormalization()(x)
        x = Dropout(0.2)(x)
        
        # Match residual shape nếu có s > 1 hoặc số lượng kênh đổi
        if residual.shape[-1] != 32 or s != 1:
            residual = Conv1D(32, 1, strides=s, padding='same')(residual)
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v23.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình MCU-Optimized TCN v23 (Strides + Pooling + No Dilation)...")
    history = model.fit(
        X_train, y_train_oh,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v23.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v23 (5 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v23 (5 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v23.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v23.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
