...
OUT_DIR = CURRENT_DIR / 'train_v14_kq_7ch'
...
def main():
    ...
    # --- TẠO DỮ LIỆU 7 TRỤC (IMU GỐC + SVM) ---
    print(f"[*] Dữ liệu gốc có shape: {X_train.shape}. Tiến hành tạo 7 trục (Accel+Gyro+SVM)...")
    X_train = add_svm_channel(X_train)
    X_val = add_svm_channel(X_val)
    X_test = add_svm_channel(X_test)
    print(f"[*] Dữ liệu sau khi tạo: {X_train.shape}")
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v14 (7 kênh - IMU + SVM)...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v14_7ch.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v14 (7 Trục - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn - TCN v14 (7 Channels)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v14_7ch.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v14_7ch.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
