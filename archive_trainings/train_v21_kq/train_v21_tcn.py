...
from tensorflow.keras.layers import Input, Conv1D, GlobalAveragePooling1D, GlobalMaxPooling1D, Concatenate, Dense, Dropout, BatchNormalization, Add, Cropping1D
...
OUT_DIR = CURRENT_DIR / 'train_v21_kq'
...
def build_tcn_mcu(input_shape, n_classes=5):
    ...
    for stack in range(2):
        for d in [1, 2, 4, 8]:
            residual = x
            # TĂNG KERNEL_SIZE LÊN 5 ĐỂ MỞ RỘNG RECEPTIVE FIELD
            x = Conv1D(32, kernel_size=5, dilation_rate=d, padding='valid', activation='relu', use_bias=True)(x)
            x = BatchNormalization()(x)
            x = Dropout(0.2)(x)
            
            # Tính toán crop_size cho kernel_size=5: (5-1)*d = 4*d
            crop_size = 4 * d
    ...
    # TINH CHỈNH POOLING: Bóp nghẹt Max Pooling để chống Spikes, giữ nguyên Average Pooling
...
def main():
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v21 (Kernel 5, GAP+GMP tinh chỉnh)...")
    history = model.fit(
        X_train, y_train,
        ...
        callbacks=callbacks,
        verbose=2 # Triệt để chống trôi log
    )
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v21.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v21 (5 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v21 (5 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v21.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v21.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
