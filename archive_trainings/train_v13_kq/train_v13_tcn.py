...
from tensorflow.keras.layers import Input, Conv1D, GlobalMaxPooling1D, Dense, Dropout, BatchNormalization, Add, Cropping1D
...
OUT_DIR = CURRENT_DIR / 'train_v13_kq'
...
def build_tcn_mcu(input_shape, n_classes=4):
    ...
    # v13: Chuyển từ Average sang Max Pooling để giữ lại đỉnh cường độ của hành động đi bộ
    x = GlobalMaxPooling1D()(x)
    x = Dropout(0.3)(x)
    outputs = Dense(n_classes, activation='softmax', name='output_layer')(x)
    model = Model(inputs=inputs, outputs=outputs)
    
    # v13: Sử dụng Label Smoothing với CategoricalCrossentropy
...
def main():
    ...
    # Tính toán class weights trên y_train dạng integer (số nguyên) trước
    class_weights_vals = class_weight.compute_class_weight(
        class_weight='balanced', classes=np.unique(y_train), y=y_train
    )
    class_weights = dict(zip(np.unique(y_train), class_weights_vals))
    
    # Giữ nguyên mức x3 cho lớp Fall, chưa tăng Walk ngay theo đề xuất v13
    fall_idx = CLASS_NAMES.index('Fall')
    class_weights[fall_idx] *= 3.0
    
    # v13: Phải chuyển nhãn sang dạng One-Hot để dùng với CategoricalCrossentropy(label_smoothing=0.1)
    y_train_onehot = tf.keras.utils.to_categorical(y_train, num_classes=len(CLASS_NAMES))
    y_val_onehot = tf.keras.utils.to_categorical(y_val, num_classes=len(CLASS_NAMES))
    
    input_shape = (X_train.shape[1], X_train.shape[2])
    model = build_tcn_mcu(input_shape, n_classes=len(CLASS_NAMES))
    
    checkpoint_path = OUT_DIR / 'best_model_v13.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v13...")
    history = model.fit(
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v13.png')
    plt.close()
    ...
    # Threshold 0.25 cho lớp Fall
    y_pred[fall_probs >= 0.25] = fall_idx
    
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v13 (Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) - TCN v13')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v13.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v13.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
        
    # tflite_path = OUT_DIR / 'model_v13_int8.tflite'
