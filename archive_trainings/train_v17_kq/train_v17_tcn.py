...
CACHE_DIR = CURRENT_DIR / 'train_cache_v16_v17_6ch' # Shared Cache
CACHE_DIR.mkdir(exist_ok=True)
OUT_DIR = CURRENT_DIR / 'train_v17_kq'
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v17.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v17 (với kiến trúc v14, 6 lớp)...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v17.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v17 (6 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v17 (6 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v17.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v17.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
