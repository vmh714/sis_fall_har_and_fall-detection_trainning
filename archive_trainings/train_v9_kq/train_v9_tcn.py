...
OUT_DIR = CURRENT_DIR / 'train_v9_kq'
...
def main():
    ...
    # Giải pháp A: Phạt cực nặng việc đoán sai nhãn Fall (index 3) - x10
    fall_idx = CLASS_NAMES.index('Fall')
    class_weights[fall_idx] *= 10.0
    ...
    checkpoint_path = OUT_DIR / 'best_model_v9.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v9...")
    history = model.fit(
        X_train, y_train,
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v9.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TRÊN TẬP KIỂM THỬ (TEST SET) - TCN v9 (Penalty x10)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Ma trận nhầm lẫn (Confusion Matrix) - TCN v9')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v9.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v9.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
        
    tflite_path = OUT_DIR / 'model_v9_int8.tflite'
