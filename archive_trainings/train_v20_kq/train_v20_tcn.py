...
# Đã gỡ bỏ đoạn cấu hình sys.stdout qua codecs.
# Việc này giúp Keras sử dụng đúng ký tự \r để đè progress bar lên 1 dòng, tránh spam CLI.
...
OUT_DIR = CURRENT_DIR / 'train_v20_kq'
...
def build_tcn_mcu(input_shape, n_classes=5):
    ...
    # KẾT HỢP GAP VÀ GMP ĐỂ BẮT ĐƯỢC CẢ NỀN (IDLE) VÀ ĐỈNH (TRANS/FALL)
...
def main():
    ...
    checkpoint_path = OUT_DIR / 'best_model_v20.keras'
    callbacks = [
    ...
    print("\n[*] Bắt đầu huấn luyện mô hình TCN v20 (GAP + GMP, Trans Weight x1.6)...")
    history = model.fit(
        X_train, y_train,
        ...
        callbacks=callbacks,
        verbose=2 # Đổi sang verbose=2: Chỉ in 1 dòng tóm tắt sau mỗi epoch, triệt để chặn spam log
    )
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'training_history_v20.png')
    plt.close()
    ...
    report_str = "\n" + "="*50 + "\n"
    report_str += "BÁO CÁO PHÂN LOẠI TẬP KIỂM THỬ - TCN v20 (5 Lớp - Threshold 0.25)\n"
    report_str += "="*50 + "\n"
    ...
    plt.title('Confusion Matrix - TCN v20 (5 Classes)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / 'confusion_matrix_v20.png')
    plt.close()
    ...
    with open(OUT_DIR / 'report_v20.txt', 'w', encoding='utf-8') as rf:
        rf.write(report_str)
